...
const HexagonBackground = () => {
  ...
  useEffect(() => {
    ...
    const buildGrid = () => {
      ...
      for (let y = -hexRadius; y < height + hexRadius; y += verticalSpacing, rowIndex++) {
        const row: Node[] = [];
        const offset = (rowIndex % 2) * (horizontalSpacing / 2);
        let colIndex = 0;

        for (let x = -horizontalSpacing; x < width + horizontalSpacing; x += horizontalSpacing, colIndex++) {
          const node: Node = {
            baseX: x + offset,
            baseY: y,
            x: x + offset,
            ...
            hexColor: colorPalette[Math.floor(Math.random() * colorPalette.length)],
          };
    ...
    const animate = (time: number) => {
      ...
      state.nodes.forEach((node) => {
        ...
        drawHexagon(node.x, node.y, state.hexRadius * 0.3 * node.hexScale);
        ctx.stroke();
      });
